...
// Changes:
//   1. Simplified status filter — COMPLETED and CANCELLED only (no kitchen flow)
//   2. Fixed: referenceNo only required/sent for non-CASH methods
//   3. Added: reprint receipt button on completed orders
...
function PaymentModal({ order, onClose, onPaid }) {
  ...
  try {
    ...
    if (needsRef && refNo.trim().length > 0) {
      payload.referenceNo = refNo.trim()
    }
  ...
  return (
    <Modal title="💳 Process Payment" onClose={onClose} width={400}>
      <div style={{ background: 'var(--brown-100)', borderRadius: 'var(--radius-md)', padding: '12px 16px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ color: 'var(--brown-800)', fontWeight: 700, fontSize: 20, fontFamily: 'var(--font-display)' }}>₱{total.toFixed(2)}</span>
      </div>
      ...
      {paid > 0 && (
        <div style={{ background: change >= 0 ? 'var(--green-light)' : 'var(--red-light)', borderRadius: 'var(--radius-md)', padding: '10px 14px', marginTop: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ color: change >= 0 ? 'var(--green-dark)' : 'var(--red-dark)', fontSize: 13, fontWeight: 600 }}>{change >= 0 ? 'Change' : '⚠ Short by'}</span>
          <span style={{ color: change >= 0 ? 'var(--green-dark)' : 'var(--red-dark)', fontWeight: 700, fontSize: 18 }}>₱{Math.abs(change).toFixed(2)}</span>
        </div>
      )}

      {needsRef && !refNo.trim() && paid >= total && (
        <p style={{ color: 'var(--brown-600)', fontSize: 12, marginTop: 10 }}>⚠ Reference number is required for {method} payments.</p>
      )}
    ...
    <Modal title={`Order ${order.orderNumber}`} onClose={onClose} width={460}>
        ...
        {order.items?.map(item => (
          <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '10px 0', borderBottom: '1px solid var(--border-light)' }}>
            ...
            <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 12 }}>
              ...
              <div style={{ fontWeight: 700, color: 'var(--brown-700)', fontSize: 14 }}>₱{Number(item.subtotal).toFixed(2)}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {order.payment && (
        <div style={{ background: 'var(--green-light)', borderRadius: 'var(--radius-md)', padding: '12px 14px', marginBottom: 16 }}>
          ...
          <div style={{ color: 'var(--green-dark)', fontSize: 13 }}>
            Paid: ₱{Number(order.payment.amountPaid).toFixed(2)} · Change: ₱{Number(order.payment.change).toFixed(2)}
          </div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        {order.status === 'PENDING' && !order.payment && (
          <Button variant="success" fullWidth onClick={() => setShowPay(true)}>
            💳 Pay Now
          </Button>
        )}
        {order.status === 'COMPLETED' && order.payment && (
          <Button variant="outline" fullWidth onClick={handleReprint}>
            🖨 Reprint Receipt
          </Button>
        )}
      </div>
...
const inputStyle = {
  ...
  borderRadius: 'var(--radius-md)', fontSize: 14, background: '#fff', outline: 'none',
}
