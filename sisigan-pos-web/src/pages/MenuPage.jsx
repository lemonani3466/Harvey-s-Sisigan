        ...
        <div style={{ background: 'var(--cream)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
          ...
          {currentItems.map((item, i) => (
            <div key={item.id} style={{
              ...
              background: !item.isAvailable ? 'rgba(0,0,0,0.02)' : undefined,
              opacity: !item.isAvailable ? 0.65 : 1,
            }}>
              <div>
                <div style={{ fontWeight: 600, color: 'var(--text-dark)', fontSize: 14 }}>{item.name}</div>
              </div>
              <div style={{ fontWeight: 700, color: 'var(--brown-700)' }}>₱{Number(item.price).toFixed(2)}</div>
